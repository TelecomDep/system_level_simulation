...
int main(int argc, char *argv[]){
    ...
    vector<cf_t> concatenate_buffer_from_matlab_to_GNB(N);
    ...
    vector<uint8_t> send_data(buffer_vec.size() * sizeof(cf_t) + 1);
     ...
     while(1){
        ...
        if(!matlab_connected){
            ...
            std::vector<char> id_data(id_size);
    
            memcpy(id_data.data(), zmq_msg_data(&id_msg), id_size);
        ...
        if (summ >= 1  && broker_rcv_accept_gnbs[0] == num_gnbs)
        {
            ...
            if (size != -1){
                ...
                cf_t overhead_byte(0, 0); //id для gNB 0 (мб будет конфликтить с UE, надо почекать)

                memcpy(send_data.data(), &overhead_byte, sizeof(cf_t));
                memcpy(send_data.data() + 1, buffer_vec.data(), send_data.size());

                send = zmq_send(socket_to_matlab, send_data.data(), size + sizeof(cf_t), 0);

                if (send == -1){
                    printf("send gNB data to matlab ERROR!!!!");
                } else {
                    printf("send gNB data to matlab SUCCEESS");
                }
            }
            ...
            for(int i = 0; i < num_ues; ++i){
                ...
                if (size != -1){
                    for(int j = 0; j < all_buffers_for_ue[i].size(); ++j){
                        concatenate_buffer[j] += all_buffers_for_ue[i][j];
                    }


                    // memcpy(send_data.data() + 1, buffer_vec.data(), send_data.size());

                    // cf_t overhead_byte(i + 1, 0);   

                    // memcpy(send_data.data(), &overhead_byte, sizeof(cf_t));

                    // send = zmq_send(socket_to_matlab, send_data.data(), nbytes, ZMQ_DONTWAIT);
            ...
            send = zmq_send(socket_to_matlab, concatenate_buffer.data(), concatenate_buffer.size() * sizeof(cf_t), 0);
        ...
        //recv from matlab
            ...
            zmq_msg_recv(&recv_data, socket, 0); //поменять название сокета

            //recv id
            zmq_msg_init(&recv_id);
            zmq_msg_recv(&recv_id, socket, 0); //поменять название сокета
            
            recv_size = zmq_msg_size(&recv_data);
            
            if (recv_size > 0) {
                //Просто приняли буфер. Надо как-то разбивать его будет еще
                concatenate_buffer_from_matlab_to_GNB.resize(recv_size);
                memcpy(concatenate_buffer_from_matlab_to_GNB.data(), zmq_msg_data(&recv_data), recv_size);

                //дописать код разбиения и распределения

                printf("Received from matlab\n");
            } else{
                printf("Nothink to recv from matlab or error \n");
            }
